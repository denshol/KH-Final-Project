...
import com.kh.final3.chatting.model.dto.ChatListDTO;
import com.kh.final3.chatting.model.dto.ChatRoomDTO;
import com.kh.final3.chatting.model.service.ChatRoomService;
import com.kh.final3.chatting.util.TimeUtils;
...
public class ChatController {
	...
	@Inject
	private MemberService guestService;
	...
	public String getWebSocketWithSockJs(Model model, 
			HttpSession session, 
			@ModelAttribute ChatRoomDTO chatRoom)
			throws Exception {
		...
		Member guest = guestService.selectMemberById(userid);
		
		
		chatRoom.setUserid(guest.getUserId());
		chatRoom.setUser_name(guest.getUserName());
...
//		}
	...
	public ModelAndView getChatList(Model model, HttpSession session, 
		...
		if(userid != null) { 
			sessionId = userid;
			g_dto=guestService.selectMemberById(userid);
		}
		...
		new Pagination();
		PageInfo pager = Pagination.getPageInfo(5, page, count, pageScale);
				//(pageScale, count, page);
...
//	}
	 ...
	 * 모두 읽었다면 ""빈값으로 리턴함
	 */
	@RequestMapping(value="/chatUnreadMessageInfo/ajax",
